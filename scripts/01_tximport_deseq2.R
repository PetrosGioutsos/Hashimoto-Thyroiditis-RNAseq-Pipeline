...
# The VST matrix is transposed so that samples are rows and genes are columns.
# dist() then computes the Euclidean distance between every pair of samples,
# giving a 9x9 symmetric distance matrix.
ssd_matrix <- dist(t(assay(vst)), method = "euclidean")
ssd_mat <- as.matrix(ssd_matrix)

# Annotation for the heatmap columns (and rows)
annot_ssd <- data.frame(
  Condition = vst$condition,
  row.names = colnames(vst)
)

# Color scale for the distance values:
# Low distance (similar samples) → dark color
# High distance (dissimilar samples) → light/white
# Direction is reversed relative to the gene heatmap, since here
# low values are more informative (samples that cluster together)
...
save_plot(
  "01_SampleDistance_HierarchicalClustering.png",
  {
    pheatmap::pheatmap(
      ssd_mat,
      clustering_distance_rows = ssd_matrix,   # uses the actual distance object,
