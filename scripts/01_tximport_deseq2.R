...
# rolls the transcript-level estimates (TPM, counts) up to gene level
# quant.sf files here come from Salmon
txi <- tximport(
  quant_files,
  type = "salmon",
...
vst <- vst(dds, blind = FALSE)  # blind = FALSE so it uses the dispersions DESeq() already estimated



# 5. LFC shrinkage 

# apeglm shrinkage — pulls in the noisy LFCs from low-count genes
...
# raw, unshrunk results — kept around for the GSEA ranking later
res_raw <- results(dds,
...
# to data frame + attach gene names/types
...
# raw (unshrunk) results, same annotation as res_df — needed for GSEA ranking
res_raw_df <- as.data.frame(res_raw)
res_raw_df$gene_id <- rownames(res_raw_df)
res_raw_df <- dplyr::left_join(res_raw_df, gene_lookup, by = "gene_id")
saveRDS(res_raw_df, file.path(paths$results, "deseq2_results_raw.rds"))
...
save_plot("01_PCA_plot.png", print(pca_plot), width = 7, height = 6)

# MA plot, shrunk LFCs
save_plot("01_MA_plot.png",
          {
            DESeq2::plotMA(
              res_shrunk,
              alpha = cfg$padj_cutoff,
...
# label with gene_name; if it's duplicated, tack on gene_id to tell them apart
  ...
  "01_Volcano_plot.png",
  {
    print(
      EnhancedVolcano::EnhancedVolcano(
        ...
        drawConnectors = TRUE,
        widthConnectors = 0.4,
        title = "Volcano plot (apeglm shrinkage)",
        subtitle = paste0("padj < ", cfg$padj_cutoff,
                          "  |LFC|  ≥ ", cfg$lfc_cutoff),
...
# Heatmap of the top 50 significant DEGs
...
if (n_top_heatmap > 0) {
  ...
  # swap gene_id rownames for gene_name where we have one
...
if (n_top_counts > 0) {
  ...
  count_plots <- lapply(seq_len(n_top_counts), function(i) {
    ...
    ggplot(cnt_df, aes(x = condition, y = count, colour = condition, fill = condition)) +
  ...
  save_plot("01_plotcounts_top6.png",
...
# transpose the VST matrix so samples are rows, genes are columns, then
# dist() gives the pairwise Euclidean distance between every sample pair
# (9x9, symmetric)
ssd_matrix <- dist(t(assay(vst)), method = "euclidean")
ssd_mat <- as.matrix(ssd_matrix)

# annotation for the heatmap rows/columns
annot_ssd <- data.frame(
  Condition = vst$condition,
  row.names = colnames(vst)
)

# color scale for the distances — dark = similar samples, light = far apart.
# this is the opposite direction from the gene heatmap above, since here
# it's the low values (samples clustering together) that matter
ssd_colors <- colorRampPalette(RColorBrewer::brewer.pal(9, "Blues"))(100)

save_plot(
  "01_SampleDistance_HierarchicalClustering.png",
  {
    pheatmap::pheatmap(
      ssd_mat,
      clustering_distance_rows = ssd_matrix,   # actual distance object,
      clustering_distance_cols = ssd_matrix,   # not a re-derived approximation
      clustering_method = "complete",   # complete linkage — conservative,
      # gives more compact clusters
      annotation_col = annot_ssd,
      annotation_row = annot_ssd,
      annotation_colors = list(Condition = palette$condition),
      color = rev(ssd_colors),       # reversed: dark = close, light = far
      border_color = NA,
      show_rownames = TRUE,
      show_colnames = TRUE,
      fontsize = 9,
      main = "Sample-to-Sample Distance\n(Euclidean  VST counts  Complete linkage)"
    )
  },
  width = 8, height = 7
)

message("Hierarchical clustering plot saved: plots/01_SampleDistance_HierarchicalClustering.png")
