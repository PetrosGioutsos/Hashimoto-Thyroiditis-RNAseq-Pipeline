...
# KEGG has limited and inconsistent coverage for non-coding genes,
# so the analysis is restricted to protein-coding DEGs.
...
# 3. ENSEMBL → Entrez ID conversion

# enrichKEGG() requires Entrez Gene IDs.
# bitr() performs the mapping; IDs without a valid Entrez entry are dropped.
...
# The universe consists of protein-coding genes that passed the rowSums >= 10
# pre-filtering step in DESeq2, converted to Entrez IDs.
# Using a biology-aware universe (tested genes only) avoids inflating enrichment
# significance with genes that were never measured in this experiment.
...
# 5. LFC vectors (Entrez-named) — used by pathview and cnetplot

# pathview and cnetplot expect a named numeric vector where:
...
# Combined LFC vector (all sig DEGs) used for pathview coloring.
# This allows the full expression context to appear on the pathway diagram,
# not just the subset of genes that drove the enrichment.
lfc_all_entrez <- c(lfc_up_entrez, lfc_down_entrez)



# 6. Gene-symbol-named LFC vectors — used by cnetplot

# After setReadable(), enrichment result tables contain gene symbols
# rather than Entrez IDs, so cnetplot expects symbol-named LFCs.
...
make_lfc_symbol <- function(mapping_df, sig_coding_df) {
  ...
  lfc <- merged$log2FoldChange
  # Fall back to Entrez ID when gene symbol is missing
  names(lfc) <- ifelse(
...
# 7. Save plot helper (consistent with 01 and 02 scripts)
...
run_kegg <- function(entrez_ids, universe_entrez, label) {
  ...
  # setReadable() converts Entrez IDs to gene symbols in the result table
  result <- tryCatch(
...
for (result_name in names(pathway_results)) {
  ...
  # --- Dotplot ---
  # Summarises enrichment ratio, gene count, and adjusted p-value in one view
  save_plot(
  ...
  # --- cnetplot ---
  # Shows which genes drive which pathways (top 5 pathways).
  # Node color reflects log2FoldChange; node size reflects gene count.
  if (n_terms >= 3 && length(lfc_vec) >= 5) {
    save_plot(
      paste0(prefix, "_cnetplot.pdf"),
...
# 11. Pathview diagrams (top 5 KEGG pathways, up and down separately)

# pathview() downloads the KEGG pathway image from the internet, overlays
# the LFC values as node colors, and saves the result as a PNG file.
# IMPORTANT: pathview always saves files into the current working directory
# (not into paths$plots). This function moves them after generation.
...
run_pathview <- function(kegg_result, lfc_entrez_vec, label, n_top = 5) {
  ...
  for (pid in top_pathways) {
    tryCatch({
      ...
      pathview::pathview(
        gene.data = lfc_entrez_vec,  # named numeric vector: Entrez ID → LFC
        pathway.id = pid,
        species = "hsa",
        out.suffix = label,           # appended to the output filename
        kegg.native = TRUE,            # renders the original KEGG pathway image
        
        # Node color gradient: blue (down) → white (unchanged) → red (up)
      ...
      # pathview saves files named "<pid>.<label>.png" (and sometimes a PDF)
      # in the working directory — move them to plots/ for consistency
